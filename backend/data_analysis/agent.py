...
def _load_app_config() -> dict:
    """Load app_config.json for credential fallback."""
...
def _pick_credential(env_name: str, config_data: dict, config_key: str) -> tuple[str | None, str]:
    """Return credential value and source label."""
    ...
    if isinstance(config_value, str) and config_value.strip():
        return config_value.strip(), "app_config"
    return None, "none"
...
def _resolve_llm_credentials() -> dict[str, str | None]:
    ...
    openai_model = (
        os.getenv("EDA_OPENAI_MODEL", "").strip()
        or str(config_data.get("eda_openai_model_name", "")).strip()
        or "gpt-4o-mini"
    )
    deepseek_model = (
        os.getenv("EDA_DEEPSEEK_MODEL", "").strip()
        or str(config_data.get("eda_deepseek_model_name", "")).strip()
        or "deepseek-chat"
    )
    deepseek_base_url = (
        os.getenv("DEEPSEEK_BASE_URL", "").strip()
        or str(config_data.get("deepseek_api_base_url", "")).strip()
...
def init_llm() -> None:
    ...
    credentials = _resolve_llm_credentials()
    openai_key = credentials.get("openai_key")
    deepseek_key = credentials.get("deepseek_key")

    if isinstance(openai_key, str) and openai_key:
        openai_model = str(credentials.get("openai_model") or "gpt-4o-mini")
        llm = ChatOpenAI(
            model=openai_model,
            temperature=0,
        ...
        log.info(
            "[init_llm] Using OpenAI (%s), key_source=%s",
            openai_model,
            credentials.get("openai_key_source"),
        )
    elif isinstance(deepseek_key, str) and deepseek_key:
        deepseek_model = str(credentials.get("deepseek_model") or "deepseek-chat")
        deepseek_base_url = str(credentials.get("deepseek_base_url") or "https://api.deepseek.com/v1")
        llm = ChatOpenAI(
        ...
        log.info(
            "[init_llm] Using DeepSeek (%s), key_source=%s",
            deepseek_model,
            credentials.get("deepseek_key_source"),
        )
    else:
        log.error("[init_llm] No API keys found in env or app_config.json")
        raise ValueError(
            "Neither OPENAI_API_KEY/DEEPSEEK_API_KEY (env) nor openai_api_key/deepseek_api_key (app_config.json) is set."
        )
